...
<img src="images/result_low.png" width="400" alt="結果画面（低得点例）"> <img src="images/result_high.png" width="400" alt="結果画面（高得点）">

▲ 結果画面：低得点例（左） / 高得点例（右）

---

## 🏗 アーキテクチャとこだわりのポイント (Engineering Highlights)
...
単一の `index.html` 上でJavaScriptによる非同期データ取得とクラス制御を行い、SPA風のサクサク動く画面遷移を実現しました。また、クイズ中のキルストリーク演出では、ブラウザのレンダリングを制御し、CSSの `@keyframes` を用いたアニメーションを実装しています。
...
~~~bash
git clone https://github.com/yourusername/lol-database-quiz.git
